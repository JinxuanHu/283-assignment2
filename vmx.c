...
/* External declaration of two counters for hw2. */
extern atomic_t num_exits;
extern atomic64_t exit_cpu_cycles;
...
static int vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	arch_atomic_inc(&num_exits);

	/* record start time of vmx_handle_exit function. */
	start_time = rdtsc();

	ret = __vmx_handle_exit(vcpu, exit_fastpath);

	/* accumulate the processing time of vmx_handle_exit function. */
	arch_atomic64_add(rdtsc() - start_time, &exit_cpu_cycles);
