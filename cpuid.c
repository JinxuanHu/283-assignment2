...
/*
 * num_exits: record number of exits of vmx_handle_exit;
 * exit_cpu_cycles: record total cpu cycles spent in vmx_handle_exit.
 */
atomic_t num_exits = ATOMIC_INIT(0);
EXPORT_SYMBOL(num_exits);
atomic64_t exit_cpu_cycles = ATOMIC64_INIT(0);
EXPORT_SYMBOL(exit_cpu_cycles);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	/**/
	if (eax == 0x4FFFFFFF) {
		printk("CPUID(0x4FFFFFFF), total number of exits: %d\n",
				arch_atomic_read(&num_exits));
		eax = arch_atomic_read(&num_exits);
		cpu_cycles = arch_atomic64_read(&exit_cpu_cycles);
		ebx = cpu_cycles >> 32;
